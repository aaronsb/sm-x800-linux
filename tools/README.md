...
| `camtest.sh` | Stream raw frames from a camera through CAMSS: links sensor -> csiphy -> csidN -> vfeN_rdi0, sets the native format on every pad, captures N packed 10-bit Bayer frames with v4l2-ctl. `sh camtest.sh uw|front|frontfull|rear [COUNT] [OUT]` on the tablet (needs v4l-utils); each camera has a default CSID/VFE pair (uw 0, front 1, rear 2; CSID N feeds IFE N only), `VFE=N CSID=N` override it. |
...
| `lens-test.sh` | DW9808 lens and module EEPROM checks on GENI I2C2 (kernel r29+): `check` greps dmesg and the media graph for the lens and at24, `lens [POS]` moves the lens standalone, `stream POS` and `sweep [POS...]` hold the lens open first and then capture rear frames with `camtest.sh` (the lens holds a position only while its subdev is open), `eeprom` dumps the nvmem head. `CAMTEST` and `OUTDIR` override the paths. On the tablet; decode frames with `raw2png.py IN.raw 4128 3096 OUT.png`. |
